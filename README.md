...
- 为每个特征映射一个或多个说明书支持段落。
- 逐条标记“特征 ↔ 段落”关系为完整支持、部分支持或需补正，并保存标记当时的段落文字摘要；段落正文修改后相关关系自动退回待复核，校验提醒列出受影响特征。
- 自动提示引用循环、缺少依据、空特征和“特征已删除但映射未清理”。
- 角色切换与批注隔离：代理人、审查员只能修改本人批注，观察者只读。
- 全量操作撤销重做，使用 `Ctrl/⌘ + Z` 与 `Ctrl/⌘ + Shift + Z`（撤销可回到上一组核对状态）。
- 保存权利要求版本（连同核对状态与依据摘要留档），逐项比较正文、状态与依据摘要差异并恢复历史版本。
- 导出完整 JSON 或可核对的 CSV（含核对状态、依据摘要、备注），本地保存上次标签页、特征和滚动位置。
